...
public class E3Sum2 {
    public static void main(String[] args) {
        ...
        List<List<Integer>> triplets = findCombinations(S, 0, 0, 3);
...
//        Arrays.sort(S);
    ...
    public static List<List<Integer>> findCombinations(int[] array, int start, int targetSum, int count) {
        ...
        if(array.length < start + count) return new LinkedList<>();
        ...
        List<List<Integer>> subCombinations = findCombinations(array, start + 1, targetSum - array[start], count - 1);
        for(List<Integer> newCombination: subCombinations) {
            newCombination.add(array[start]);
            ...
            if(!duplicate) combinations.add(newCombination);
        }
