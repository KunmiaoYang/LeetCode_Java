...
public class Solution1 {
    ...
    public boolean isScramble(String s1, String s2) {
        ...
        countMap1 = new int[n + 1][128];
        countMap2 = new int[n + 1][128];
        for(int i = 0; i < n; i++) {
            System.arraycopy(countMap1[i], 0, countMap1[i + 1], 0, 128);
            System.arraycopy(countMap2[i], 0, countMap2[i + 1], 0, 128);
            countMap1[i + 1][chars1[i]]++;
            countMap2[i + 1][chars2[i]]++;
        }
        return isScramble(0, n - 1, 0, n - 1);
    }
    boolean isScramble(int l1, int r1, int l2, int r2) {
        if(l1 == r1 && chars1[l1] == chars2[l2]) return true;
        for(int i = 0; i < 128; i++) if(countMap1[r1 + 1][i] - countMap1[l1][i] != countMap2[r2 + 1][i] - countMap2[l2][i]) return false;
        int n = r1 - l1;
        for(int i = 0; i < n; i++) {
